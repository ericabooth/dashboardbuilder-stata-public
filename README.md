...
![A dashboardbuilder dashboard titled "Texas county readiness explorer" with three tabs (Readiness map, Two-variable map, The numbers); the Readiness map tab shows a sparkta2 blue choropleth of all 254 Texas counties with a readiness legend and a reset-zoom control, embedded as a panel inside the dashboard card, with a Save as PDF button floating in the bottom-right corner.](images/example-embedded-map.png)

This dashboard is generated by [`example_dashboardbuilder_map.do`](example_dashboardbuilder_map.do): two `sparkta2` D3 maps of all 254 Texas counties (a choropleth, shown, and a bivariate two-variable cut on the next tab), each dropped into a card with the `html` panel type. The maps are inlined, so the whole dashboard is still one self-contained file. (The Save as PDF button sits in the bottom-right corner here via the `corner` build option.) See it live in the [demo gallery](https://ericabooth.github.io/dashboardbuilder_Example_Site/).
