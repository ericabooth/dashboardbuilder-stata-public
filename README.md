...
![A dashboardbuilder dashboard titled "Texas county readiness explorer" with Map and "The numbers" tabs and a Save as PDF button; the Map tab shows a sparkta2 blue choropleth of all 254 Texas counties with a readiness legend and a reset-zoom control, embedded as a panel inside the dashboard card.](images/example-embedded-map.png)

This dashboard is generated by [`example_dashboardbuilder_map.do`](example_dashboardbuilder_map.do): a `sparkta2` D3 choropleth of all 254 Texas counties, dropped into a card with the `html` panel type. The map is inlined, so the whole dashboard is still one self-contained file. See it live in the [demo gallery](https://ericabooth.github.io/dashboardbuilder_Example_Site/).
