...
def _dbb_assemble():
    try:
        ...
        if sel:
            static = [p["title"] or p["id"] for p in panels if not p["selcol"]]
            if static:
                todo.append("selector is on, but these panels lack a %s column and stay static: %s" % (sel, "; ".join(static)))
...
(function scaffold(){
  ...
  DASH.tabs.forEach(t=>{
    ...
    DASH.panels.filter(p=>p.tab===t.name).forEach(p=>{
      const c=document.createElement("div"); c.className="card"; c.id="panelcard_"+p.id;
